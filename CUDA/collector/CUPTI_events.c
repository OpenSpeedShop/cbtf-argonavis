...
void CUPTI_events_start(CUcontext context)
{
    ...
    if (IsDebugEnabled)
    {
        printf("[CUDA %d:%d] CUPTI_events_start(%p)\n",
    ...
    for (i = 0; (i < MAX_CONTEXTS) && (Events.values[i].context != NULL); ++i)
    {
        ...
        {
            fprintf(stderr, "[CUDA %d:%d] CUPTI_events_start(): "
    ...
    for (e = 0; e < TheSamplingConfig.events.events_len; ++e)
    {
        ...
        if (IsDebugEnabled)
        {
            printf("[CUDA %d:%d] recording GPU event \"%s\" for context %p\n",
...
void CUPTI_events_stop(CUcontext context)
{
    ...
    if (IsDebugEnabled)
    {
        printf("[CUDA %d:%d] CUPTI_events_stop(%p)\n",
               getpid(), monitor_get_thread_num(), context);
    }
#endif
    ...
    if (i == MAX_CONTEXTS)
    {
        fprintf(stderr, "[CUDA %d:%d] CUPTI_events_stop(): "
